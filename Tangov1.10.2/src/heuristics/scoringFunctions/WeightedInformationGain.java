...
public class WeightedInformationGain extends Heuristic {
	 ...
	 */
		...
		int tpx = nowcm.getTruePositives();
	 ...
	 */
		...
		int t = 2*tp + tn + 2* cm.getFalsePositives() + cm.getFalseNegatives();

	    result = Utilities.log2(t+1) - Utilities.log2((2*tp)+tn+1);
		return result;
	}
	 ...
	 */
			...
			result = (tpnow + tnnow) <= 0 ? 0.0 : ((((2*tpnow + totneg + 1) * x) - (2*tpnow + totneg + fnnow + 1)) / (1+x));
