...
public class Popolo extends Heuristic {
	...
	public double score(Node<Literal,InformationContext> node) {
		...
		if (tpx != 0 && fp != 0) { // if fp == 0 means that the negative coverage has not been computed yet
	...
	public double getMaxPossibleGain(Node<Literal,InformationContext> currentNode) {
		...
		// fp1 must be 0 to reach the best score
		if (tp != 0)
